...
function groupHandler(
  ...
  maxHeight,
  maxWidth,
  leftPositionSubtractor = 0,
  topPositionSubtractor = 0
) {
...
function containerHandler(
  ...
  let bottomLeftBorderRadiusCoef =
    containerObject["bottomLeftRadius"] / maxHeight;
  let bottomRightBorderRadiusCoef =
    containerObject["bottomRightRadius"] / maxHeight;
  ...
  if (containerObject["strokes"].length > 0) {
    ...
    let borderColorRedGamma = Math.round(
      containerObject["strokes"]["0"]["color"]["r"] * 255
    );
    let borderColorGreenGamma = Math.round(
      containerObject["strokes"]["0"]["color"]["g"] * 255
    );
    let borderColorBlueGamma = Math.round(
      containerObject["strokes"]["0"]["color"]["b"] * 255
    );
    let borderColorOpacity = containerObject["strokes"]["0"]["opacity"].toFixed(
      2
    );
    borderObject = `
                                border: Border.all(
  ...
  if (
    containerObject.effects.length > 0 &&
    containerObject["fills"]["0"]["type"] == "SOLID"
  ) {
    let shadowOpacity = containerObject["effects"]["0"]["color"]["a"].toFixed(
      2
    );
    let shadowColorRedGamma = Math.round(
      containerObject["effects"]["0"]["color"]["r"] * 255
    );
    let shadowColorGreenGamma = Math.round(
      containerObject["effects"]["0"]["color"]["g"] * 255
    );
    let shadowColorBlueGamma = Math.round(
      containerObject["effects"]["0"]["color"]["b"] * 255
    );
    let shadowHorizontalOffsetCoef =
      containerObject["effects"]["0"]["offset"]["x"] / maxWidth;
    let shadowVerticalOffsetCoef =
      containerObject["effects"]["0"]["offset"]["y"] / maxHeight;
    let shadowBlurRadiusCoef =
      containerObject["effects"]["0"]["radius"] / maxHeight;
    shadowObject = `
                                boxShadow: [
                                    BoxShadow(
...
function textHandler(
  ...
  let fontColorRedGamma = Math.round(
    textObject["fills"]["0"]["color"]["r"] * 255
  );
  let fontColorGreenGamma = Math.round(
    textObject["fills"]["0"]["color"]["g"] * 255
  );
  let fontColorBlueGamma = Math.round(
    textObject["fills"]["0"]["color"]["b"] * 255
  );
...
function ellipseHandler(
  ...
  if (ellipseObject["strokes"].length > 0) {
    ...
    let borderColorRedGamma = Math.round(
      ellipseObject["strokes"]["0"]["color"]["r"] * 255
    );
    let borderColorGreenGamma = Math.round(
      ellipseObject["strokes"]["0"]["color"]["g"] * 255
    );
    let borderColorBlueGamma = Math.round(
      ellipseObject["strokes"]["0"]["color"]["b"] * 255
    );
    let borderColorOpacity = ellipseObject["strokes"]["0"]["opacity"].toFixed(
      2
    );
    borderObject = `
                                border: Border.all(
  ...
  if (ellipseObject.effects.length > 0) {
    ...
    let shadowColorRedGamma = Math.round(
      ellipseObject["effects"]["0"]["color"]["r"] * 255
    );
    let shadowColorGreenGamma = Math.round(
      ellipseObject["effects"]["0"]["color"]["g"] * 255
    );
    let shadowColorBlueGamma = Math.round(
      ellipseObject["effects"]["0"]["color"]["b"] * 255
    );
    let shadowHorizontalOffsetCoef =
      ellipseObject["effects"]["0"]["offset"]["x"] / maxWidth;
    let shadowVerticalOffsetCoef =
      ellipseObject["effects"]["0"]["offset"]["y"] / maxHeight;
    let shadowBlurRadiusCoef =
      ellipseObject["effects"]["0"]["radius"] / maxHeight;
    shadowObject = `
                                boxShadow: [
                                    BoxShadow(
                                        color: Color.fromRGBO( ${shadowColorRedGamma}, ${shadowColorGreenGamma}, ${shadowColorBlueGamma},${shadowOpacity}),
...
for (let node of figma.currentPage["children"]) {
  if (node.locked == false) {
    ...
    backgroundColorRedGamma = Math.round(
      node["fills"]["0"]["color"]["r"] * 255
    );
    backgroundColorGreenGamma = Math.round(
      node["fills"]["0"]["color"]["g"] * 255
    );
    backgroundColorBlueGamma = Math.round(
